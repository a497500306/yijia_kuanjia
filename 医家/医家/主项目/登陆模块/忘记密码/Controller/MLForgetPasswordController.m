...
-(void)chushihua{
    ...
    shoujiField.tintColor = [UIColor colorWithRed:3/255.0 green:166/255.0 blue:116/255.0 alpha:1];
    ...
    yanzhenmaField.tintColor = [UIColor colorWithRed:3/255.0 green:166/255.0 blue:116/255.0 alpha:1];
    ...
    queding.backgroundColor = [UIColor colorWithRed:3/255.0 green:166/255.0 blue:116/255.0 alpha:1];
    queding.tag = 100;
    [queding.layer setBorderWidth:1.0f];
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    CGColorRef colorref = CGColorCreate(colorSpace,(CGFloat[]){ 3/255.0,166/255.0, 116/255.0,1.0 });
...
-(void)dianjimeishoudao{
    ...
    //网络发送验证码
    //发送验证码
    [SMSSDK getVerificationCodeByMethod:SMSGetCodeMethodSMS phoneNumber:self.shoujiField.text zone:@"86" customIdentifier:nil result:^(NSError *error) {
        if (error == nil) {
            //发送成功,做跳转
...
-(void)dianjixiayibu{
    ...
    //网络发送修改密码请求
    //验证验证码
    [SMSSDK commitVerificationCode:self.yanzhemaField.text phoneNumber:self.shoujiField.text zone:@"86" result:^(NSError *error) {
        if (error == nil) {//验证成功,发送修改密码网络请求
            [MBProgressHUD hideHUDForView:self.view];
            NSString *mima = [MD5Tool md5:[self.mimaField.text stringByAppendingString:@"rolle"]];
            NSDictionary *dict = @{@"password":mima,@"phone":self.shoujiField.text};
            NSDictionary *params = @{@"params":[MLJson json:dict]};
            [IWHttpTool postWithURL:[MLInterface xgmm] params:params success:^(id json) {
            ...
            //发送失败,给提示
            [MBProgressHUD hideHUDForView:self.view];
            [MBProgressHUD showError:error.userInfo[@"commitVerificationCode"] toView:self.view];
        }
    }];
}
#pragma mark - 单击退出键盘
