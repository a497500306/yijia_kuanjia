...
-(void)dianjixiayibu{
    ...
    //验证验证码
    [SMSSDK commitVerificationCode:self.yanzhemaField.text phoneNumber:self.shouji zone:@"86" result:^(NSError *error) {
        if (error == nil) {
            MLIDVerificationController *vc = [[MLIDVerificationController alloc] init];
            vc.shouji = self.shouji;
            vc.mima = mima;
