...
- (void)setupSameUI
{
    ...
    //创建背景图
    UIImageView *iamgeView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
    iamgeView.image = [UIImage imageNamed:@"手势密码_背景图1"];
...
- (void)setupSubViewsSettingVc
{
    ...
    //添加退出按钮
    UIButton * btn = [[UIButton alloc] initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width - 40)/2, [UIScreen mainScreen].bounds.size.height - 60 , 40, 40)];
    [btn setImage:[UIImage imageNamed:@"手势密码_取消"] forState:UIControlStateNormal];
