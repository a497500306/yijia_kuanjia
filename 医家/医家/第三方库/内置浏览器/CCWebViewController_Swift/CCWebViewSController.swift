...
class CCWebViewSController: UIViewController,UIWebViewDelegate,UIActionSheetDelegate,WKNavigationDelegate {
    ...
    /** 传入控制器、url、标题 */
    class func showWithContro(contro: UIViewController,withUrlStr url: NSString,withTitle title: String, isTool:Bool) -> Void {
    ...
    func configUI() {
        ...
        if #available(iOS 8.0, *) {
            ...
            if self.isTool == false {//没有工具栏
                wkWebView.frame = self.view.bounds
            }else{//有评论点赞栏
                wkWebView.frame = CGRectMake(0, 0, self.view.frame.width, UIScreen.mainScreen().bounds.height - 44)
            }
            ...
            if self.isTool == false {//没有工具栏
                ...
                webView.frame = CGRectMake(0, 0, self.view.frame.width, UIScreen.mainScreen().bounds.height - 44)
            }
